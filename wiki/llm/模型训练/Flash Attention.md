...
![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image.png|image.png]]
...
> 其中 ${x_1, \dots, x_N}$ 就是 $QK^T$，也就是 `pre-softmax logits`。

1. 提前计算好 $x$，保存在 HBM 中，需要 $O(N^2)$ 显存，可能会爆显存。
2. online 计算 $x_i$，每次循环 load QK 到 SRAM，计算 $x_i$。

![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 1.png|image.png]]
...
![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 2.png|image.png]]
...
![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 3.png|image.png]]
...
![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 4.png|image.png]]

![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 5.png|image.png]]
...
- $d'_i$ 的更新：
    ...
    $$
    d'_i \leftarrow d'_{i-1} e^{m_{i-1}-m_i} + \sum e^{x_i[j]-m_i}
    $$
    

- 这里使用了 **指数修正**。因为最大值 $m_i$ 可能已经变了，所以需要将旧的分母 $d'_{i-1}$ 乘以一个缩放因子 $e^{m_{i-1}-m_i}$，再加上当前块的指数和。

- $o'_i$ 的修正同理。$d'_i$ 和 $m_i$ 更新后，要在旧项上乘上修正因子：
...
![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/Snipaste_2026-02-03_15-40-37.png|Snipaste_2026-02-03_15-40-37.png]]

![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/Snipaste_2026-02-03_15-40-42.png|Snipaste_2026-02-03_15-40-42.png]]

![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 6.png|image.png]]

## FlashAttention V1工程实现

![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 7.png|image.png]]

- 前向传播：online-softmax和tilling优化

![[raw/assets/notion导出知识库/知识库/LLM/模型训练/Flash Attention/image 8.png|image.png]]
