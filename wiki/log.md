---
title: log
summary: "记录本地 Obsidian wiki 的迁移、结构调整、索引维护与内容整理历史。"
note_type: log
area: wiki
topic: log
collection: wiki
...
- 修复 `wiki/` 中一批不符合 Obsidian Markdown 的格式问题：为空代码块补语言标识，清理旧式 inline math delimiters 和坏掉的 `file:///` / 外链混合链接，并手工修正 `cs336`、`Flash Attention`、`Agent面试文档（2026-2-23）` 的公式与代码块渲染。
...
- 将 [[raw/weixin/Vector Graph RAG 开源！一套向量数据库同时搞定语义检索+RAG多跳|raw/weixin/Vector Graph RAG 开源！一套向量数据库同时搞定语义检索+RAG多跳.md]] 整理为 [[wiki/llm/RAG/Vector Graph RAG|wiki/llm/RAG/Vector Graph RAG.md]]，归入 `RAG` 主题目录，并补充到首页导航。
...
- 知识库巡检修复新检出环境中的检索初始化缺口：`./qmdw sync` 现在会自动补建缺失的 `wiki` / `raw` collection，再执行 update 与 embedding；同步修正 [[wiki/qmdw|qmdw]] 中失效的旧机器绝对索引路径，并增加回归测试覆盖首次同步调用顺序。
