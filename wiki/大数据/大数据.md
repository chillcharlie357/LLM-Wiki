---
title: 大数据
summary: "面向学习与面试准备的大数据知识主线：从分布式存储、批流计算和数据治理的核心抽象出发，理解 HDFS、HBase、Spark、Kafka、Flink 与湖仓架构的选型、故障边界和常见追问。"
...
本文不按课程周次或考试章节组织，而是围绕面试最常考察的三种能力展开：

- **讲清原理**：能从问题出发解释组件为什么存在，而不是只背名词。
- **做出取舍**：能比较吞吐、延迟、一致性、成本、治理与运维复杂度。
- **落到工程**：能用数据流、状态、故障与观测指标回答系统设计追问。

初学时按“学习路线”逐层推进；准备面试时用“问题链”和“系统设计框架”复述，每个结论都尽量回答 **适用场景、核心机制、失败方式、替代方案** 四件事。原课程仓库保留为延伸阅读和原理出处，不作为本文结构。
...
| 阶段 | 学习目标 | 能力验收 |
| --- | --- | --- |
| 1. 分布式基础 | 掌握 [[wiki/foundations/计算机网络|网络]]、磁盘 I/O、分片、复制、选主、重试、幂等与一致性 | 能解释“节点必然会失败”如何改变存储与计算设计 |
| 2. 批处理体系 | 沿 `HDFS -> MapReduce -> YARN -> Spark` 理解数据本地性、Shuffle、DAG 与资源调度 | 能画出一次作业的数据流并定位慢 Stage |
| 3. 流处理体系 | 沿 `Kafka -> Flink` 理解分区日志、事件时间、状态、Checkpoint 与端到端语义 | 能解释乱序、重复、积压和恢复后的正确性 |
| 4. 分析与治理 | 沿 `Hive/Parquet -> 数仓 -> 数据湖 -> Table Format` 理解表、文件、元数据和治理 | 能根据读写模式选择架构并说明小文件、Schema 演进和权限边界 |

最后用一个端到端项目闭环：采集业务事件，经 Kafka 写入实时链路，以 Flink 聚合并写入分析存储，同时保留 HDFS/对象存储上的离线明细，再补齐监控、重放、幂等和成本估算。项目重点不是把组件都部署一遍，而是能解释每一层解决了什么问题、失败时如何恢复。

遇到性能问题时，先定位瓶颈属于 **CPU、内存、磁盘、网络、Shuffle、数据倾斜、元数据还是下游背压**；遇到正确性问题时，先画清 **数据边界、状态边界、重试边界与提交边界**。

## 10. 面试准备：用问题链组织答案

面试回答不要停在定义。推荐使用“**场景 → 机制 → 取舍 → 故障/优化**”四步结构：先说明组件解决的问题，再讲关键数据路径，然后给出边界，最后接住追问。

| 高频问题 | 回答主线 | 常见追问 |
| --- | --- | --- |
| HDFS 为什么适合大文件、不适合小文件？ | Block、副本、顺序吞吐与 NameNode 内存元数据 | 小文件如何治理？HA 与 Federation 分别解决什么？ |
| HBase 写入为什么快？ | WAL + MemStore + HFile 的 LSM 写路径，以顺序写换后台 Compaction | RowKey 热点、读放大、Major Compaction 风险 |
| MapReduce、Spark 如何选择？ | 两者都依赖分区和 Shuffle；Spark 用 DAG、流水线与缓存减少重复落盘 | 宽窄依赖、Stage 划分、数据倾斜与 OOM |
| Kafka 如何保证顺序与不丢数据？ | 顺序边界是 Partition；副本、ACK、ISR 和 Offset 共同决定可靠性 | 扩分区后的 key 映射、重复消费、Exactly-once 边界 |
| Flink 如何处理乱序和故障恢复？ | Event Time + Watermark 决定计算进度，State + Checkpoint 恢复一致状态 | 迟到数据、背压、两阶段提交、Checkpoint 变慢 |
| 数仓、数据湖、湖仓有什么区别？ | 比较 Schema 时机、存储开放性、事务表语义与治理能力 | ODS/DWD/DWS/ADS、Table Format、Catalog 与小文件 |

回答组件对比题时，先统一比较维度，避免罗列功能：**数据模型、读写路径、延迟、吞吐、状态与一致性、容错方式、扩展方式、运维成本**。如果题目缺少数据规模或 SLA，应主动询问，而不是直接报技术名。

## 11. 系统设计题：从需求到方案

以“实时指标平台”一类题目为例，可以按以下顺序展开：

1. **澄清约束**：事件量、峰值吞吐、端到端延迟、保留周期、查询模式、可接受的数据误差。
2. **画数据流**：Producer → Kafka → Flink → OLAP/湖仓，并说明离线回放和补数路径。
3. **定义分区与状态**：选择业务 key、Partition 数、窗口、状态 TTL、Watermark 与迟到策略。
4. **说明正确性**：明确 At-most-once、At-least-once 或 Exactly-once；把 source、checkpoint、sink 和外部副作用放在同一条提交链上讨论。
5. **处理容量与故障**：估算存储和带宽，说明扩容、倾斜、积压、节点故障、消息重复与下游不可用时的策略。
6. **补齐可观测性**：监控 consumer lag、反压、checkpoint 时长/失败率、吞吐、延迟分位数、数据新鲜度和对账差异。

这套框架的目标是展示推理过程：从业务约束推导架构，而不是把熟悉的组件拼成固定答案。
